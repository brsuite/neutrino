...
import (
	...
	"github.com/brsuite/neutrino/chainsync"
	"github.com/brsuite/neutrino/headerfs"
	"github.com/brsuite/neutrino/headerlist"
...
type blockManagerCfg struct {
	...
	GetBlock func(chainhash.Hash, ...QueryOption) (*bronutil.Block, error)
...
func resolveFilterMismatchFromBlock(block *wire.MsgBlock,
	...
	for peerAddr, filter := range filtersFromPeers {
		...
		numOpReturns, err := VerifyBasicBlockFilter(
			filter, bronutil.NewBlock(block),
		)
		if err != nil {
...
func (b *blockManager) checkHeaderSanity(blockHeader *wire.BlockHeader,
	...
	stubBlock := bronutil.NewBlock(&wire.MsgBlock{
		Header: *blockHeader,
	})
