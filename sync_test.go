...
import (
	...
	"github.com/brsuite/neutrino/banman"
	"github.com/brsuite/neutrino/headerfs"
)

var (
	// Try bronlog.LevelInfo for output like you'd see in normal operation,
	// or bronlog.LevelTrace to help debug code. Anything but
	// bronlog.LevelOff turns on log messages from the tests themselves as
	// well. Keep in mind some log messages may not appear in order due to
	// use of multiple query goroutines in the tests.
	logLevel    = bronlog.LevelOff
	...
	ourKnownTxsByBlock = make(map[chainhash.Hash][]*bronutil.Tx)
...
type secSource struct {
	keys    map[string]*bronec.PrivateKey
	scripts map[string]*[]byte
	params  *chaincfg.Params
}

func (s *secSource) add(privKey *bronec.PrivateKey) (bronutil.Address, error) {
	pubKeyHash := bronutil.Hash160(privKey.PubKey().SerializeCompressed())
	addr, err := bronutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.params)
	if err != nil {
		return nil, err
	}
...
func (s *secSource) GetKey(addr bronutil.Address) (*bronec.PrivateKey, bool,
	error) {
...
func (s *secSource) GetScript(addr bronutil.Address) ([]byte, error) {
...
func newSecSource(params *chaincfg.Params) *secSource {
	return &secSource{
		keys:    make(map[string]*bronec.PrivateKey),
...
var (
	...
	secSrc                    *secSource
	addr1, addr2, addr3       bronutil.Address
...
func testRescan(harness *neutrinoHarness, t *testing.T) {
	...
	newPkScript := func() (bronutil.Address, []byte, *wire.TxOut) {
		t.Helper()
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target bronutil.Amount) (
		total bronutil.Amount, inputs []*wire.TxIn,
		...
		return func(target bronutil.Amount) (total bronutil.Amount,
			inputs []*wire.TxIn, inputValues []bronutil.Amount,
			...
			inputValues = []bronutil.Amount{
				bronutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	privKey3, err := bronec.NewPrivateKey(bronec.S256())
	if err != nil {
		t.Fatalf("Couldn't generate private key: %s", err)
	}
	...
	_, err = harness.h1.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
		[]*bronutil.Tx{}, rpctest.BlockVersion, time.Time{},
		[]wire.TxOut{{
			Value:    0,
			PkScript: []byte{},
		}})
...
func TestNeutrinoSync(t *testing.T) {
	// Set up logging.
	logger := bronlog.NewBackend(os.Stdout)
	...
	// Create a brond SimNet node and generate 800 blocks
	h1, err := rpctest.New(
	...
	// Create a second brond SimNet node
	h2, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"}, "",
	)
	if err != nil {
	...
	// Create a third brond SimNet node and generate 1200 blocks
	h3, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"}, "",
	)
	if err != nil {
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr bronutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRedeemingTx: func(tx *bronutil.Tx,
					details *bronjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
