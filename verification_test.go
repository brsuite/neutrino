...
func TestVerifyBlockFilter(t *testing.T) {
	...
	b := bronutil.NewBlock(spendBlock)
...
func spendP2PK(_ *testing.T, _ *bronec.PublicKey, prevTx *wire.MsgTx,
...
func spendP2PKH(_ *testing.T, pubKey *bronec.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendP2SH(t *testing.T, pubKey *bronec.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendNP2WKH(t *testing.T, pubKey *bronec.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {

	pkHash := bronutil.Hash160(pubKey.SerializeCompressed())

	witAddr, err := bronutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func spendP2WKH(_ *testing.T, pubKey *bronec.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendP2WSH(t *testing.T, pubKey *bronec.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func makeP2PK(t *testing.T, pubKey *bronec.PublicKey) []byte {
	addr, err := bronutil.NewAddressPubKey(
...
func makeP2PKH(t *testing.T, pubKey *bronec.PublicKey) []byte {
	pkHash := bronutil.Hash160(pubKey.SerializeCompressed())
	addr, err := bronutil.NewAddressPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2SH(t *testing.T, pubKey *bronec.PublicKey) []byte {
	script := scriptP2PKH(t, pubKey)

	addr, err := bronutil.NewAddressScriptHash(script, chainParams)
	require.NoError(t, err)
...
func makeNP2WKH(t *testing.T, pubKey *bronec.PublicKey) []byte {
	pkHash := bronutil.Hash160(pubKey.SerializeCompressed())
	...
	addr, err := bronutil.NewAddressScriptHash(witnessProgram, chainParams)
	require.NoError(t, err)
...
func makeP2WKH(t *testing.T, pubKey *bronec.PublicKey) []byte {
	pkHash := bronutil.Hash160(pubKey.SerializeCompressed())

	addr, err := bronutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2WSH(t *testing.T, pubKey *bronec.PublicKey) []byte {
...
func scriptP2PKH(t *testing.T, pubKey *bronec.PublicKey) []byte {
...
func incrementKey(key *bronec.PublicKey) *bronec.PublicKey {
