...
import (
	...
	"github.com/brsuite/neutrino/filterdb"
	...
	s.queryPeers(
		...
		func(sp *ServerPeer, resp wire.Message,
			...
			case *wire.MsgBlock:
				...
				block := bronutil.NewBlock(response)

				// Only set height if bronutil hasn't
				// automagically put one in.
				if block.Height() == bronutil.BlockHeightUnknown {
